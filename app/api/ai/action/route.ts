...
async function openRouterCall(model: string, systemPrompt: string, userPrompt: string, apiKey: string) {
  // Sanitize model name to remove unexpected dots, spaces, or punctuation generated by the orchestrator
  // Note: Added '.' to the regex to preserve model names like 'claude-3.5-sonnet'
  const sanitizedModel = model.replace(/[^a-zA-Z0-9\-\/\.]/g, '').trim();
