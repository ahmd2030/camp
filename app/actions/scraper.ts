...
export async function scrapeGooglePlaces(searchQuery: string, defaultStatus: 'PENDING' | 'READY_TO_SEND' = 'PENDING') {
  try {
    ...
    const finalLeads = await Promise.all(leadsToProcess.map(async (lead) => {
      ...
      try {
        const prompt = `أنت مدير مبيعات استراتيجي (B2B) في وكالة حلول رقمية مرموقة.
النشاط المستهدف: ${lead.businessName}.
نقطة الألم لديهم: ${lead.painPoint}.

اكتب رسالة بريد إلكتروني احترافية (Cold Email) موجهة لإدارة هذه الشركة، مبنية على هيكل PAS (Problem, Agitation, Solution).
يجب أن تتضمن الرسالة الإلزاميات التالية:
1. تحية احترافية لإدارة ${lead.businessName}.
2. Problem (المشكلة): اذكر نقطة الألم بصيغة استشارية راقية (مثال: "من خلال تحليلنا الأولي لتواجدكم الرقمي، لاحظنا...").
3. Agitation (التأثير): الفت انتباههم بذكاء إلى ما يخسرونه بسبب هذه المشكلة (خسارة عملاء محتملين، ضعف الثقة مقارنة بالمنافسين).
4. Solution (الحل): قدم نظامنا/برنامجنا كحل جذري وآلي لتجاوز هذه العقبة.
5. CTA (الدعوة لاتخاذ إجراء): ضع [LINK] بأسلوب احترافي لكي يضغطوا عليه لرؤية الحل.

اختم الرسالة دائماً بالتوقيع التالي حرفياً ولا تستخدم أي أقواس أو متغيرات فارغة:
فريق المبيعات الذكي
Mango AI
الرائدة في الحلول الرقمية

النبرة: استشارية، رسمية، واثقة، وتدل على شركة متكاملة الأطراف. يجب أن يكون النص باللغة العربية الفصحى الاحترافية ومختصراً قدر الإمكان لتجنب الملل.`;
